...
class Socket : public std::streambuf
{
  ...
  typedef std::unique_ptr<struct addrinfo, AddrinfoDestructor>
                    AddrinfoContainer;
  AddrinfoContainer getSocketInfoFor(const char* host, unsigned int port,
  ...
  /*! Shifts the remaining data to the beginning of the input intermediate
   buffer
      and fills up the input intermediate buffer by receiving data (TCP)
      or writes the next received packet at the beginning of the input
   intermediate buffer (UDP)
...
public:
   ...
   */
  void initAsTcpClient(const std::string& hostRemote, unsigned portRemote,
                       bool waitUntilConnected = false);

  /*! Setup socket as TCP server
   @param hostLocal The host to be listening to:

   "" or "*" to listen to any incoming data (IPVersion will be choosen by
   system)
   ...
   */
  void initAsTcpServer(const std::string& hostLocal, unsigned portLocal,
  ...
  /*! Returns only the number of outstanding bytes to be received from the
   system cache
   @return Number of bytes in the system cache, not including iostream buffers
   @warning Use in_avail() instead if you are interested in the total number of
  ...
  */
  ...
  /*! Receives size bytes into buffer
   ...
   @warning In most cases you don't want to call this directly but use the
   iostream API instead (see wiki)
  */
  ...
  */
  ...
  /*! Redirects received data to all Sockets in destinations
   ...
   @warning If one destination can't handle the outgoing load, a Expection is
   thrown and all other destinations get out of sync (data loss)
  */
  std::streamsize redirect(
    const std::vector<std::shared_ptr<Socket> >& destinations);
  ...
  //! Set the size of the input intermediate buffer in bytes (unread contents
  //! are lost)
  void setInputBufferSize(std::streamsize size);
  //! Set the size of the output intermediate buffer in bytes (unwritten
  //! contents are lost)
  ...
  /*! Updates the blocking mode of the socket.
   ...
   Try to avoid using blocking mode. Use a SocketManager with listen(sec > 0.0)
   instead.
   */
  ...
  //! Disconnects the socket, deletes the intermediate buffers and sets the
  //! handle to -1
  void disconnect();
  //! Check if there is a problem with this socket and disconnect in case there
  //! is one
