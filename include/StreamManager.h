...
{
protected:
  typedef std::pair<Element*, int64_t>
    StackElement; //!< To store the (de)serializer position
  std::unique_ptr<Element>
                            rootElement; //!< The element at the root of the hierarchy
...
{
  typedef std::function<bool(std::unique_ptr<Element> parsedElement)>
    PushCallback; //!< Typedef of callback to return deserialized elements
public:
  /*! Constructs the Deserializer
   @param _streamBuffer A std::streambuf to be used as target for write
   operations
   */
  ...
  /*! Deserializes elements from the streamBuffer
   @param pushElement Callback which will be called when the next element has
                      been deserialized and can return true to stop the
   deserialization
   @param hierarchy If false arrays and maps will be deserialized as a flat
   stream of elements
   @param bytes Limit of bytes to read or 0 to read as much as possible
   */
  std::streamsize deserialize(PushCallback pushElement, bool hierarchy = true,
                              std::streamsize bytes = 0);
  /*! Deserializes one element from the streamBuffer
   ...
   @param bytes Limit of bytes to read or 0 to read as much as possible
   */
  std::streamsize deserialize(std::unique_ptr<Element>& element,
                              bool hierarchy = true, std::streamsize bytes = 0);
