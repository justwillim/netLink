...
//! Decrements a Utf8 string iterator string by n characters or stops at end
//! iterator
template <typename iterator>
iterator&
backwardUntil(iterator& i, const iterator& end, size_t n)
{
...
template <typename iterator>
iterator&
forwardUntil(iterator& i, const iterator& end, size_t n)
{
  for (size_t c = 0; i < end; ++i)
  {
    if ((*i & 0xC0) != 0x80)
...
template <typename iterator>
size_t
difference(const iterator& end, const iterator& begin)
{
  size_t len = 0;
...
//! Erase a interval from a Utf8 string at the given character position and
//! length
...
start:
  ...
  switch (countLeadingOneBits(c << 24))
  {
    ...
    case 4:
      return (c & 0x0F) << 18 | (i[1] & 0x3F) << 12 | (i[2] & 0x3F) << 6 |
             (i[3] & 0x3F);
    default:
      return -1;
  }
