...
{
public:
  //! Event which is called if a TCP_SERVER accepts a new connection (if false
  //! is returned the connection will be closed immediately)
  std::function<bool(SocketManager*          manager,
                     std::shared_ptr<Socket> serverSocket,
                     std::shared_ptr<Socket> clientSocket)>
    onConnectRequest;
  //! Event which is called if a socket can or can not send more data (also
  //! called if nonblocking connect succeeded)
  std::function<void(SocketManager* manager, std::shared_ptr<Socket> socket,
                     Socket::Status prev)>
    onStatusChange;
  ...
  /*! Listens a periode time
   @param waitUpToSeconds Maximum time to wait for incoming data in seconds or
   negative values to wait indefinitely
   */
