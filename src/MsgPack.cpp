...
void
storeUint8(char* target, uint8_t source)
{
  *reinterpret_cast<uint8_t*>(target) = source;
}

void
storeInt8(char* target, int8_t source)
{
  *reinterpret_cast<int8_t*>(target) = source;
}

void
storeUint16(char* target, uint16_t source)
{
  storeBSwap(16);
}

void
storeInt16(char* target, int16_t source)
{
  storeBSwap(16);
}

void
storeFloat32(char* target, float source)
{
  storeUint32(target, reinterpret_cast<const uint32_t&>(source));
}

void
storeUint32(char* target, uint32_t source)
{
  storeBSwap(32);
}

void
storeInt32(char* target, int32_t source)
{
  storeBSwap(32);
}

void
storeFloat64(char* target, double source)
{
  storeUint64(target, reinterpret_cast<const uint64_t&>(source));
}

void
storeUint64(char* target, uint64_t source)
{
  storeBSwap(64);
}

void
storeInt64(char* target, int64_t source)
{
  storeBSwap(64);
}

uint8_t
loadUint8(const char* source)
{
  return *reinterpret_cast<const uint8_t*>(source);
}

int8_t
loadInt8(const char* source)
{
  return *reinterpret_cast<const int8_t*>(source);
}

uint16_t
loadUint16(const char* source)
{
  uint16_t target;
  loadBSwap(16);
}
...
Primitive::Primitive(Type _type)
  : type(_type)
{
}

Primitive::Primitive(bool value)
  : type((value) ? Type::BOOL_TRUE : Type::BOOL_FALSE)
{
}
...
int64_t
Primitive::startDeserialize(uint8_t firstByte)
{
  type = firstByte;
  return 1;
}
...
std::streamsize
Primitive::serialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                     std::streamsize bytes)
{
  if (bytes > 0 && pos == 0 &&
      streamBuffer->sputc(type) != std::streambuf::traits_type::eof())
    return ++pos;
  else
    return 0;
}
...
void
Primitive::toJSON(std::ostream& stream) const
{
  switch (type)
  {
    case Type::NIL:
...
int64_t
Header::startDeserialize(uint8_t firstByte)
{
  header[0] = firstByte;
...
std::streamsize
Header::serialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                  std::streamsize bytes)
{
  if (pos < 0)
  {
...
std::streamsize
Header::deserialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                    std::streamsize bytes)
{
  if (pos < 0)
  {
...
uint32_t
Header::getSizeInBytes() const
{
  return static_cast<uint32_t>(getHeaderLength() + getEndPos());
}

uint32_t
Header::getLength() const
{
  return static_cast<uint32_t>(getEndPos());
}

std::streamsize
Data::serialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                std::streamsize bytes)
{
...
std::streamsize
Data::deserialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                  std::streamsize bytes)
{
...
void
Binary::toJSON(std::ostream& stream) const
{
  auto len = getLength();
...
int64_t
Extended::getHeaderLength() const
{
  uint8_t type = static_cast<const uint8_t>(header[0]);
  switch (type)
  {
...
void
Extended::toJSON(std::ostream& stream) const
{
  auto len = getLength();
...
uint8_t
Extended::getDataType() const
{
  return data[0];
}

char*
Extended::getData() const
{
  return &data[1];
}

uint32_t
Extended::getLength() const
{
  return getEndPos() - 1;
}

void
String::init(uint32_t len, const void* str)
{
  if (len > 0)
  {
    data.reset(new char[len]);
...
void
String::toJSON(std::ostream& stream) const
{
  stream << "\"";
...
int64_t
Number::startDeserialize(uint8_t firstByte)
{
  data[0] = firstByte;
  return 1;
}
...
std::streamsize
Number::serialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                  std::streamsize bytes)
{
  bytes = std::min(bytes, (std::streamsize)(getEndPos() - pos));
...
std::streamsize
Number::deserialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer,
                    std::streamsize bytes)
{
  bytes = std::min(bytes, (std::streamsize)(getEndPos() - pos));
...
void
Number::toJSON(std::ostream& stream) const
{
  uint8_t type = static_cast<const uint8_t>(data[0]);
  if (type < FIXMAP)
    stream << (int16_t)type;
...
bool
Number::isUnsignedInteger() const
{
  uint8_t type = static_cast<const uint8_t>(data[0]);
...
std::unique_ptr<Element>
ArrayHeader::copy() const
{
  return std::unique_ptr<Element>(new ArrayHeader(getLength()));
}

void
ArrayHeader::toJSON(std::ostream& stream) const
{
  stream << "< Array length=" << getLength() << " >";
}

uint32_t
ArrayHeader::getSizeInBytes() const
{
  return static_cast<uint32_t>(getHeaderLength());
}

int64_t
ArrayHeader::getHeaderLength() const
{
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXARRAY && type < Type::FIXSTR)
    return 1;
...
int64_t
MapHeader::getHeaderLength() const
{
  uint8_t type = static_cast<const uint8_t>(header[0]);
...
std::unique_ptr<Element>
MapHeader::copy() const
{
  return std::unique_ptr<Element>(new MapHeader(getLength()));
}

void
MapHeader::toJSON(std::ostream& stream) const
{
  stream << "< Map length=" << getLength() << " >";
}

uint32_t
MapHeader::getSizeInBytes() const
{
  return static_cast<uint32_t>(getHeaderLength());
}

uint32_t
MapHeader::getLength() const
{
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXMAP && type < Type::FIXARRAY)
...
Array::Array(std::vector<std::unique_ptr<Element> >&& _elements)
  : ArrayHeader(_elements.size())
  , elements(std::move(_elements))
{
}

bool
Array::containerDeserialized()
{
...
void
Array::toJSON(std::ostream& stream) const
{
  ...
  : MapHeader(_elements.size())
{
  ...
  {
    elements.push_back(
      std::unique_ptr<Element>(new MsgPack::String(pair.first)));
    elements.push_back(std::move(pair.second));
  }
}

bool
Map::containerDeserialized()
{
  uint32_t len = getLength();
  if (len > 0)
  {
...
void
Map::toJSON(std::ostream& stream) const
{
...
std::vector<std::unique_ptr<Element> >*
Map::getElementsVector()
{
  return &elements;
}

std::map<std::string, Element*>
Map::getElementsMap() const
{
...
std::pair<String*, Element*>
Map::getEntry(uint32_t index) const
{
  index = index * 2;
  if (index + 1 < elements.size())
  {
...
String*
Map::getKey(uint32_t index) const
{
  index = index * 2;
  return (index < elements.size())
           ? dynamic_cast<String*>(elements[index].get())
           : nullptr;
}

Element*
Map::getValue(uint32_t index) const
{
  index = index * 2 + 1;
  return (index < elements.size()) ? elements[index].get() : nullptr;
}
...
std::streamsize
Serializer::serialize(PullCallback pullElement, std::streamsize bytesLeft)
{
  ...
  while (bytesLeft > 0 || deserializeAll)
  {
    ...
    if (stack.size() == 0)
    {
      ...
      stack.push_back(
        StackElement(rootElement.get(), rootElement->startSerialize()));
    }
    ...
    // Serialize element
    std::streamsize bytesWritten = stackPointer->first->serialize(
      stackPointer->second, streamBuffer, bytesLeft);
    ...
    std::vector<std::unique_ptr<Element> >* container =
      stackPointer->first->getElementsVector();
    if (container && container->size() > 0)
    {
      // Serialized header, begin with first child
      Element* childElement = container->begin()->get();
      stack.push_back(
        StackElement(childElement, childElement->startSerialize()));
      continue;
    }
...
std::streamsize
Serializer::serialize(std::unique_ptr<Element>& element,
                      std::streamsize           bytesLeft)
{
  return (element)
           ? serialize([&element]() { return std::move(element); }, bytesLeft)
           : 0;
}

std::streamsize
Deserializer::deserialize(PushCallback pushElement, bool hierarchy,
                          std::streamsize bytesLeft)
{
  bool            deserializeAll = (bytesLeft == 0);
  std::streamsize bytesDone      = 0;
  ...
  while (bytesLeft > 0 || deserializeAll)
  {
    ...
    StackElement* stackPointer =
      (stack.size() > 0) ? &stack[stack.size() - 1] : NULL;

    if (stackPointer && stackPointer->second < stackPointer->first->getEndPos())
    {
      // Deserialize element
      std::streamsize bytesRead = stackPointer->first->deserialize(
        stackPointer->second, streamBuffer, bytesLeft);
      bytesLeft -= bytesRead;
      bytesDone += bytesRead;
    ...
    else
    {
      ...
      if (stack.size() > 0)
      {
        std::vector<std::unique_ptr<Element> >* container =
          stackPointer->first->getElementsVector();
        *(container->begin() + stackPointer->second) =
          std::move(std::unique_ptr<Element>(element));
      }
      else
    ...
    if (stack.size() > 0)
    {
      ...
      while (true)
      {
        ...
        if (container &&
            stack[stackIndex].second + 1 < (int64_t)container->size())
        {
...
std::streamsize
Deserializer::deserialize(std::unique_ptr<Element>& element, bool hierarchy,
                          std::streamsize bytesLeft)
{
  return deserialize(
...
std::ostream&
operator<<(std::ostream& ostream, const Element& obj)
{
  obj.toJSON(ostream);
  return ostream;
}
