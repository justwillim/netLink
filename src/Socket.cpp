...
static void
readSockaddr(const struct sockaddr_storage* addr, std::string& host,
             unsigned int& port)
{
  ...
  else
  {
    ...
    inet_ntop(addr->ss_family, (void*)&(sin->sin6_addr), buffer,
              sizeof(buffer));
  }
  host = buffer;
}

Socket::AddrinfoContainer
Socket::getSocketInfoFor(const char* host, unsigned int port,
                         bool wildcardAddress)
{
...
Socket::pos_type
Socket::seekoff(Socket::off_type off, std::ios_base::seekdir way,
...
Socket::pos_type
Socket::seekpos(Socket::pos_type sp, std::ios_base::openmode which)
{
...
std::streamsize
Socket::xsgetn(char_type* buffer, std::streamsize size)
{
...
std::streamsize
Socket::xsputn(const char_type* buffer, std::streamsize size)
{
...
void
Socket::initSocket(bool blockingConnect)
{
  ...
  {
    handle =
      socket(nextAddr->ai_family, nextAddr->ai_socktype, nextAddr->ai_protocol);
    if (handle == -1)
    {
...
#else
    int flag = 1;
    if (setsockopt(handle, SOL_SOCKET, SO_REUSEPORT,
    ...
    {
      int flag = 0;
      if (setsockopt(handle, IPPROTO_IPV6, IPV6_V6ONLY,
                     reinterpret_cast<const char*>(&flag), sizeof(flag)) == -1)
      {
        disconnect();
        throw Exception(Exception::ERROR_SET_SOCK_OPT);
    ...
    switch (type)
    {
      ...
      case TCP_CLIENT:
        if (connect(handle, nextAddr->ai_addr, nextAddr->ai_addrlen) == -1 &&
            blockingConnect)
        {
          closesocket(handle);
          handle = -1;
...
#endif
  if (getsockname(handle, reinterpret_cast<struct sockaddr*>(&localAddr),
                  &size) != 0)
  {
    disconnect();
...
void
Socket::initAsTcpServer(const std::string& _hostLocal, unsigned _portLocal,
...
void
Socket::initAsUdpPeer(const std::string& _hostLocal, unsigned _portLocal)
{
  type      = UDP_PEER;
  hostLocal = _hostLocal;
  portLocal = _portLocal;
...
std::streamsize
Socket::receive(char_type* buffer, std::streamsize size)
{
  if (type == TCP_SERVER)
    throw Exception(Exception::BAD_TYPE);
...
#endif
      int result =
        recvfrom(handle, (char*)buffer, size, 0,
                 reinterpret_cast<struct sockaddr*>(&remoteAddr), &addrSize);
      if (result <= 0)
      {
        portRemote = 0;
        hostRemote = "";
...
std::streamsize
Socket::send(const char_type* buffer, std::streamsize size)
{
  if (type == TCP_SERVER)
    throw Exception(Exception::BAD_TYPE);
  ...
  switch (type)
  {
    ...
    {
      AddrinfoContainer info =
        getSocketInfoFor(hostRemote.c_str(), portRemote, false);
      size_t sentBytes = 0;
      while (sentBytes < (size_t)size)
      {
        int result =
          ::sendto(handle, (const char*)buffer + sentBytes, size - sentBytes, 0,
    ...
    case TCP_SERVERS_CLIENT:
    {
      ...
      {
        int result =
          ::send(handle, (const char*)buffer + sentBytes, size - sentBytes, 0);
        if (result <= 0)
        {
          status = BUSY;
          throw Exception(Exception::ERROR_SEND);
...
std::streamsize
Socket::redirect(const std::vector<std::shared_ptr<Socket> >& destinations)
{
  if (type == TCP_SERVER)
    throw Exception(Exception::BAD_TYPE);
...
std::streamsize
Socket::getInputBufferSize()
{
  return inputIntermediateSize;
}

std::streamsize
Socket::getOutputBufferSize()
{
  return epptr() - pbase();
}

void
Socket::setInputBufferSize(std::streamsize n)
{
  if (eback())
    delete[] eback();
...
void
Socket::setOutputBufferSize(std::streamsize n)
{
  if (pbase())
    delete[] pbase();
  if (type != TCP_SERVER && n > 0)
...
void
Socket::setBroadcast(bool active)
{
...
void
Socket::setMulticastGroup(const std::string& address, bool join)
{
  if (type != UDP_PEER)
    throw Exception(Exception::BAD_PROTOCOL);
  if (ipVersion == IPv4)
  {
    ...
    if ((ntohl(sin.s_addr) & 0xF0000000) == 0xE0000000)
    {
      ...
      if (setsockopt(handle, IPPROTO_IP,
  ...
  else
  {
    ...
    if (sin.s6_addr[0] == 0xFF)
    {
      ...
      if (setsockopt(
            handle, IPPROTO_IPV6, (join) ? IPV6_JOIN_GROUP : IPV6_LEAVE_GROUP,
...
Socket::accept()
{
  ...
  int clientHandle = ::accept(
    handle, reinterpret_cast<struct sockaddr*>(&remoteAddr), &addrSize);
  if (clientHandle == -1)
    return nullptr;
...
Socket::disconnectOnError()
{
  ...
  getsockopt(handle, SOL_SOCKET, SO_ERROR, reinterpret_cast<char*>(&error),
             &length);
#else
