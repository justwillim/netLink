...
main(int argc, char** argv)
{
  ...
  try
  {
    // Init socket as UDP listening to all incoming adresses (let the system
    // choose IPv4 or IPv6) on port 3824
  ...
  // Define the destination for the next sent message (depending on the choosen
  // IP version)
  socket->hostRemote = (socket->getIPVersion() == netLink::Socket::IPv4)
                         ? "224.0.0.100"
                         : "FF02:0001::";
  ...
  msgPackSocket << MsgPack::Factory("test", 3);
