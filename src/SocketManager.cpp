...
#define foreach_e(c, i)                                                        \
  for (auto end##i = (c).end(), next##i = (c).begin(),                         \
            i          = (next##i == end##i) ? end##i : next##i++;             \
       i != next##i; i = (next##i == end##i) ? end##i : next##i++)
...
std::shared_ptr<Socket>
SocketManager::newMsgPackSocket()
{
...
void
SocketManager::listen(double waitUpToSeconds)
{
